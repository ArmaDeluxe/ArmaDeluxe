...
_title = format[localize "STR_ISTR_Crow_Process",if(!_isVehicle) then {"Handcuffs"} else {getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")}];
...
if(!_isVehicle) then {
    ...
    _dice = random(30);
    if(_dice < 30) then {
        titleText[localize "STR_ISTR_Crow_Success","PLAIN"];
