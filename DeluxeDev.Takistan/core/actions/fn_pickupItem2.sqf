...
if (_lockedGold) exitWith {
	["<t size='3.2' color='#e57300'>This gold bar belongs to a safe which is still locked!</t>"] call life_fnc_alerttwo;
};
...
if(_diff <= 0) exitWith {
	["<t size='3.2' color='#e57300'>Can't pick up that item, you are full.</t>"] call life_fnc_alerttwo;
};
...
else
{
	if(([true,_itemInfo select 0,_itemInfo select 1] call life_fnc_handleInv)) then
	{
		deleteVehicle _obj;
		player playmove "AinvPknlMstpSlayWrflDnon";
		[format["You picked up %1 %2",_diff,_itemName], false] spawn RLG_fnc_RLGNotificationSystem;
	};
};